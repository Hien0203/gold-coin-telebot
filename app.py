...
def get_stock_price(symbol: str):
  try:
    ...
    if res.status_code != 200:
      return f"❌ Không thể kết nối tới SSI iBoard cho mã `{symbol}`."

    json_data = res.json()
    if json_data.get("code") != "SUCCESS" or not json_data.get("data"):
      return f"❌ Không tìm thấy thông tin mã `{symbol}` trên SSI iBoard."

    data = json_data["data"]

    company_name = data.get("companyNameVi", symbol)
...
↕️ Cao/Thấp: {high_price:,.0f} / {low_price:,.0f} (Phiên: {session})
🌐 Khối ngoại: Mua {foreign_buy:,.0f} | Bán {foreign_sell:,.0f} CP"""

  except Exception as e:
    print("SSI API FETCH ERROR:", e)
    return f"❌ Lỗi khi lấy dữ liệu `{symbol}` từ SSI iBoard."
...
def start(message):
  bot.reply_to(
      ...
      "- Gõ /token <mã> hoặc /stock <mã> để tra bảng giá SSI (Ví dụ: `/token"
      " mbb`)",
      parse_mode="Markdown",
  )
...
def gold(message):
  ...
  try:
    ...
    bot.edit_message_text(
        chat_id=message.chat.id, message_id=msg.message_id, text=text
    )
  except Exception as e:
    print("BOT ERROR:", e)
